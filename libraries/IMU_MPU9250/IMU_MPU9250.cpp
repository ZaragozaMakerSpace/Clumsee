...
void MPU9250::setRate(uint8_t rate) {
    MPU9250::writeByte(MPU9250_ADDRESS, SMPLRT_DIV, rate);
}

void MPU9250::setFullScaleGyroRange(uint8_t range) {
    MPU9250::writeBits(MPU9250_ADDRESS, GYRO_CONFIG, GCONFIG_FS_SEL_BIT, GCONFIG_FS_SEL_LENGTH, range);
}

uint8_t MPU9250::getFullScaleAccelRange() {
    uint8_t buffer[14];
    MPU9250::readBits(MPU9250_ADDRESS, ACCEL_CONFIG, ACONFIG_AFS_SEL_BIT, ACONFIG_AFS_SEL_LENGTH, buffer);
    return buffer[0];
}

void MPU9250::setFullScaleAccelRange(uint8_t range) {
    MPU9250::writeBits(MPU9250_ADDRESS, ACCEL_CONFIG, ACONFIG_AFS_SEL_BIT, ACONFIG_AFS_SEL_LENGTH, range);
}

//Byte Registers to access data
...
bool MPU9250::writeByte(uint8_t address, uint8_t subAddress, uint8_t data){
  return writeBytes(address, subAddress, 1, &data);
}

bool MPU9250::writeBytes(uint8_t address, uint8_t subAddress, uint8_t length, uint8_t* data) {
  Wire.beginTransmission(address);  // Initialize the Tx buffer
  Wire.write(subAddress);           // Put slave register address in Tx buffer
...
bool MPU9250::writeBits(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t data) {
    ...
    if (readByte(devAddr, regAddr, &b) != 0) {
        ...
        return writeByte(devAddr, regAddr, data);
    } else {
        return false;
    }
}
...
uint8_t MPU9250::readByte(uint8_t address, uint8_t subAddress, uint8_t * data )
{ 
  Wire.beginTransmission(address);         // Initialize the Tx buffer
  Wire.write(subAddress);                  // Put slave register address in Tx buffer
  Wire.endTransmission(false);             // Send the Tx buffer, but send a restart to keep connection alive
  Wire.requestFrom(address, (uint8_t) 1);  // Read one byte from slave register address 
  uint8_t value = (uint8_t) Wire.read();                      // Fill Rx buffer with result
  data = &value;
  return value;
}
